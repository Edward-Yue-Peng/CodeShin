...
def evaluate_code_with_gpt(description,user_code, history, related_topics):
    ...
    如果用户的解法相较存在可提升之处，请给出较为具体的分析，例如代码复杂度、可读性等方面的建议。如果用户的解法已经很优秀，请给予充分的肯定和鼓励。
    如果用户的代码无法运行或远离正确解决方案，请指出具体问题并给出改进建议。
    如果用户未提交代码或未提交完整代码，请给出相应的提示。
    查看用户向你询问的历史记录，如果用户过度向你求助，可酌情提示让用户自主思考，并扣除一定分数。
