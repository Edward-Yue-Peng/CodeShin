...
def call_gpt_for_growth_path(user_id):
    ...
    for record in user_last_scores:
        ...
        problem_url = f"{settings.BASE_URL}/api/get_problems/"
        problem_response = requests.get(problem_url, params={"id": problem_id})
        
        if problem_response.status_code != 200:
            return {"error": f"Failed to fetch problem data for problem_id {problem_id}: {problem_response.json().get('error', 'Unknown error')}"}
    ...
    # Step 4: 调用 GPT API
    gpt_payload = {
        "model": settings.MODEL,  # 使用 settings 中的模型配置
        "messages": [
            {
                "role": "system",
    ...
    gpt_headers = {
        "Authorization": f"Bearer {settings.API_KEY}",  # 使用 settings 中的 API 密钥
        "Content-Type": "application/json"
    }

    try:
        gpt_response = requests.post(settings.URL, json=gpt_payload, headers=gpt_headers)  # 使用 settings 中的 URL
        gpt_response.raise_for_status()
        gpt_reply = gpt_response.json()["choices"][0]["message"]["content"]
        return {"suggestions": gpt_reply}
    except requests.RequestException as e:
        return {"error": f"Error calling GPT API: {str(e)}"}